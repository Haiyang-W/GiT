...
class DeformableLocalDetrTransformerDecoderLayer(DetrTransformerDecoderLayer):
    """Decoder layer of Deformable DETR."""

    def _init_layers(self) -> None:
        """Initialize cross-attn, ffn, and norms."""
        self.cross_attn = MultiScaleDeformableAttention(**self.cross_attn_cfg)
...
class DeformableLocalDetrTransformerDecoder(DeformableDetrTransformerDecoder):
    """Transformer LocalDecoder of Deformable DETR."""

    def _init_layers(self) -> None:
        """Initialize decoder layers."""
        self.layers = ModuleList([
            DeformableLocalDetrTransformerDecoderLayer(**self.layer_cfg)
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims
        if self.post_norm_cfg is not None:
    ...
    def forward(self,
        ...
        layer_start_end_index = torch.cat([level_start_index, level_start_index.new_tensor([value.shape[1]])])
        output = query
        intermediate = []
        intermediate_reference_points = []
        for layer_id, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                ...
                reference_points_input = \
                    reference_points[:, :, None] * \
                    valid_ratios[:, None]
            layer_start_index = layer_start_end_index[layer_id]
            layer_end_index = layer_start_end_index[layer_id+1]
            output = layer(
                output,
                query_pos=query_pos,
                value=value[:, layer_start_index: layer_end_index, :],
                key_padding_mask=key_padding_mask[:, layer_start_index: layer_end_index],
                spatial_shapes=spatial_shapes[layer_id: layer_id+1],
                level_start_index=level_start_index[0:1],
                valid_ratios=valid_ratios[:, layer_id: layer_id+1],
                reference_points=reference_points_input[:, :, layer_id: layer_id+1, :],
                **kwargs)
