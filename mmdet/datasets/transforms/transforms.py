...
class AddMetaInfo(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        task_name = self.meta_dict['task_name']
        if 'classes' in results:
            results['classes'] = list(results['classes'])
        if task_name in ['detection', 'instance_segmentation']:
            use_vocab_list = results['classes'] + ['background'] + [str(i) for i in range(self.meta_dict['head_cfg']['num_bins']+1)]
        elif task_name == 'semantic_segmentation':
            use_vocab_list = results['classes'] + ['background','background1','background2']
        elif task_name == 'grounding':
            use_vocab_list = [str(i) for i in range(self.meta_dict['head_cfg']['num_bins']+1)]
